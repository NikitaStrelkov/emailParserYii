...
class EmailController extends Controller
{
    ...
    /**
     * Function returns all all emails which have been found on page $url
     * @param $url
     * @return array()
     */
    ...
    private function getEmailsFromDB ($url) {
        $emailRows = Yii::app()->db->createCommand()
            ->select('*')
            ->from('tbl_email')
            ->where('site=:site', array(':site'=>$url))
            ->queryAll();
        $emails = array();
    ...
    {
        $model = new Email;
        $insertedEmails = array(); // this array need to avoid doubles
        $emails = array(); // result array
        if (isset($_POST["Email"])) {
            ...
            if (empty($emails)) {
                ...
                foreach ($emails as $email) {
                    if (!in_array($email, $insertedEmails)) {
                        ...
                        $command->insert('tbl_email', array(
                            'site' => $url,
                            'email'=> $email,
